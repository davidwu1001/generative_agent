...
def save_to_time_cost_txt(func_name, old, new):
  with open("/data/wcc/generative_agent/log/time_cost.txt", 'a') as f:
    f.write(f"{func_name} {round(new - old, 3)}\n")
...
def run_gpt_prompt_wake_up_hour(persona, test_input=None, verbose=False):
  ...
  save_to_time_cost_txt(run_gpt_prompt_wake_up_hour.__name__, old, time.time())
...
def run_gpt_prompt_daily_plan(persona, 
  ...
  if debug or verbose:
    print_run_prompts(prompt_template, persona, gpt_param,
                      prompt_input, prompt, output)
  save_to_time_cost_txt(run_gpt_prompt_daily_plan.__name__, old, time.time())
  return output, [output, prompt, gpt_param, prompt_input, fail_safe]
...
def run_gpt_prompt_generate_hourly_schedule(persona,
  ...
  if debug or verbose:
    print_run_prompts(prompt_template, persona, gpt_param,
                      prompt_input, prompt, output)
  save_to_time_cost_txt(run_gpt_prompt_generate_hourly_schedule.__name__, old, time.time())
...
def run_gpt_prompt_task_decomp(persona,
  ...
  def __func_clean_up(gpt_response, prompt=""):
    ...
    for item in previous_items:
      _cr += [" ".join([j.strip() for j in item.split(" ")][4:])]
  ...
  save_to_time_cost_txt(run_gpt_prompt_task_decomp.__name__, old, time.time())
  return output, [output, prompt, gpt_param, prompt_input, fail_safe]
...
def run_gpt_prompt_action_sector(action_description,
  ...
  gpt_param = {"engine": "text-davinci-002", "max_tokens": 15,
               "temperature": 0, "top_p": 1, "stream": False,
               "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
  prompt_template = "persona/prompt_template/v4/action_location_sector.txt"
  prompt_input = create_prompt_input(action_description, persona, maze)
  prompt = generate_prompt(prompt_input, prompt_template)
  ...
  if debug or verbose:
    print_run_prompts(prompt_template, persona, gpt_param,
                      prompt_input, prompt, output)
  save_to_time_cost_txt(run_gpt_prompt_action_sector.__name__, old, time.time())
  return output, [output, prompt, gpt_param, prompt_input, fail_safe]
...
def run_gpt_prompt_action_arena(action_description,
  ...
  def __func_validate(gpt_response, prompt=""):
    if len(gpt_response.strip()) < 1 or "}" not in gpt_response or "," in gpt_response or not __func_clean_up(gpt_response):
      return False
    else:
      return True
  ...
  gpt_param = {"engine": "text-davinci-003", "max_tokens": 15,
               "temperature": 0, "top_p": 1, "stream": False,
               "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
  prompt_template = "persona/prompt_template/v4/action_location_object.txt"
  ...
  if debug or verbose: 
    print_run_prompts(prompt_template, persona, gpt_param, 
                      prompt_input, prompt, output)
  save_to_time_cost_txt(run_gpt_prompt_action_arena.__name__, old, time.time())
...
def run_gpt_prompt_action_game_object(action_description, 
  ...
  if debug or verbose: 
    print_run_prompts(prompt_template, persona, gpt_param, 
                      prompt_input, prompt, output)
  save_to_time_cost_txt(run_gpt_prompt_action_game_object.__name__, old, time.time())
...
def run_gpt_prompt_pronunciatio(action_description, persona, verbose=False):
  ...
  output = ChatGPT_safe_generate_response(prompt, example_output, special_instruction, 3, fail_safe,
                                          __chat_func_validate, __chat_func_clean_up, True)
  save_to_time_cost_txt(run_gpt_prompt_pronunciatio.__name__, old, time.time())
  if output != False:
    return output, [output, prompt, gpt_param, prompt_input, fail_safe]
  # ChatGPT Plugin ===========================================================
...
def run_gpt_prompt_event_triple(action_description, persona, verbose=False):
  ...
  if debug or verbose: 
    print_run_prompts(prompt_template, persona, gpt_param, 
                      prompt_input, prompt, output)
  save_to_time_cost_txt(run_gpt_prompt_event_triple.__name__, old, time.time())
  return output, [output, prompt, gpt_param, prompt_input, fail_safe]
...
def run_gpt_prompt_act_obj_desc(act_game_object, act_desp, persona, verbose=False):
  ...
  output = ChatGPT_safe_generate_response(prompt, example_output, special_instruction, 3, fail_safe,
                                          __chat_func_validate, __chat_func_clean_up, True)
  save_to_time_cost_txt(run_gpt_prompt_act_obj_desc.__name__, old, time.time())
  if output != False:
    return output, [output, prompt, gpt_param, prompt_input, fail_safe]
...
def run_gpt_prompt_new_decomp_schedule(persona, 
  ...
  def __func_clean_up(gpt_response, prompt=""):
    # 删掉~
    gpt_response = gpt_response[gpt_response.find("~") + 1:]
    ...
    x = x.split(" to ")[1].strip().split()[0]

    ret = []
    for idx, (time_str, action) in enumerate(ret_temp):
      start_time = time_str.split(" ~ ")[0].strip()
      # 最后一行特殊处理一下
      end_time = time_str.split(" ~ ")[1].strip() if idx != len(ret_temp) - 1 else x
  ...
  save_to_time_cost_txt(run_gpt_prompt_new_decomp_schedule.__name__, old, time.time())
  return output, [output, prompt, gpt_param, prompt_input, fail_safe]
...
def run_gpt_prompt_decide_to_talk(persona, target_persona, retrieved,test_input=None, 
  ...
  def __func_validate(gpt_response, prompt=""): 
    try: 
      if __func_clean_up(gpt_response) in ["yes", "no"]:
        return True
      return False     
    except:
      return False 
  ...
  def __func_clean_up(gpt_response, prompt=""):
    return gpt_response.strip().split(' ')[0].lower().strip('.')
  ...
  gpt_param = {"engine": "text-davinci-003", "max_tokens": 20, 
               "temperature": 0, "top_p": 1, "stream": False,
               "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
  prompt_template = "persona/prompt_template/v4/decide_to_talk.txt"
  ...
  save_to_time_cost_txt(run_gpt_prompt_decide_to_talk.__name__, old, time.time())
  return output, [output, prompt, gpt_param, prompt_input, fail_safe]
...
def run_gpt_prompt_decide_to_react(persona, target_persona, retrieved,test_input=None, 
  ...
  save_to_time_cost_txt(run_gpt_prompt_decide_to_react.__name__, old, time.time())
  return output, [output, prompt, gpt_param, prompt_input, fail_safe]
...
def run_gpt_prompt_create_conversation(persona, target_persona, curr_loc,
  ...
  save_to_time_cost_txt(run_gpt_prompt_create_conversation.__name__, old, time.time())
  return output, [output, prompt, gpt_param, prompt_input, fail_safe]
...
def run_gpt_prompt_summarize_conversation(persona, conversation, test_input=None, verbose=False):
  ...
  def __func_clean_up(gpt_response, prompt=""):
    if "conversing about " in gpt_response.strip():
  ...
  output = ChatGPT_safe_generate_response(prompt, example_output, special_instruction, 3, fail_safe,
                                          __chat_func_validate, __chat_func_clean_up, True)
  save_to_time_cost_txt(run_gpt_prompt_summarize_conversation.__name__, old, time.time())
  if output != False:
    return output, [output, prompt, gpt_param, prompt_input, fail_safe]
  # ChatGPT Plugin ===========================================================
...
def run_gpt_prompt_extract_keywords(persona, description, test_input=None, verbose=False):
  ...
  save_to_time_cost_txt(run_gpt_prompt_extract_keywords.__name__, old, time.time())
  return output, [output, prompt, gpt_param, prompt_input, fail_safe]
...
def run_gpt_prompt_keyword_to_thoughts(persona, keyword, concept_summary, test_input=None, verbose=False):
  ...
  if debug or verbose: 
    print_run_prompts(prompt_template, persona, gpt_param, 
                      prompt_input, prompt, output)
  save_to_time_cost_txt(run_gpt_prompt_keyword_to_thoughts.__name__, old, time.time())
  return output, [output, prompt, gpt_param, prompt_input, fail_safe]
...
def run_gpt_prompt_convo_to_thoughts(persona, 
  ...
  save_to_time_cost_txt(run_gpt_prompt_convo_to_thoughts.__name__, old, time.time())
  return output, [output, prompt, gpt_param, prompt_input, fail_safe]
...
def run_gpt_prompt_event_poignancy(persona, event_description, test_input=None, verbose=False):
  ...
  save_to_time_cost_txt(run_gpt_prompt_event_poignancy.__name__, old, time.time())
  if output != False:
    return output, [output, prompt, gpt_param, prompt_input, fail_safe]
  # ChatGPT Plugin ===========================================================
...
def run_gpt_prompt_thought_poignancy(persona, event_description, test_input=None, verbose=False):
  ...
  output = ChatGPT_safe_generate_response(prompt, example_output, special_instruction, 3, fail_safe,
                                          __chat_func_validate, __chat_func_clean_up, True)
  save_to_time_cost_txt(run_gpt_prompt_thought_poignancy.__name__, old, time.time())
  if output != False:
    return output, [output, prompt, gpt_param, prompt_input, fail_safe]
  # ChatGPT Plugin ===========================================================
...
def run_gpt_prompt_chat_poignancy(persona, event_description, test_input=None, verbose=False):
  ...
  output = ChatGPT_safe_generate_response(prompt, example_output, special_instruction, 3, fail_safe,
                                          __chat_func_validate, __chat_func_clean_up, True)
  save_to_time_cost_txt(run_gpt_prompt_chat_poignancy.__name__, old, time.time())
  if output != False:
    return output, [output, prompt, gpt_param, prompt_input, fail_safe]
  # ChatGPT Plugin ===========================================================
...
def run_gpt_prompt_focal_pt(persona, statements, n, test_input=None, verbose=False):
  ...
  save_to_time_cost_txt(run_gpt_prompt_focal_pt.__name__, old, time.time())
  return output, [output, prompt, gpt_param, prompt_input, fail_safe]
...
def run_gpt_prompt_insight_and_guidance(persona, statements, n, test_input=None, verbose=False):
  ...
  save_to_time_cost_txt(run_gpt_prompt_insight_and_guidance.__name__, old, time.time())
  return output, [output, prompt, gpt_param, prompt_input, fail_safe]
...
def run_gpt_prompt_agent_chat_summarize_ideas(persona, target_persona, statements, curr_context, test_input=None, verbose=False):
  ...
  output = ChatGPT_safe_generate_response(prompt, example_output, special_instruction, 3, fail_safe,
                                          __chat_func_validate, __chat_func_clean_up, True)
  save_to_time_cost_txt(run_gpt_prompt_agent_chat_summarize_ideas.__name__, old, time.time())
  if output != False:
    return output, [output, prompt, gpt_param, prompt_input, fail_safe]
  # ChatGPT Plugin ===========================================================
...
def run_gpt_prompt_agent_chat_summarize_relationship(persona, target_persona, statements, test_input=None, verbose=False):
  ...
  output = ChatGPT_safe_generate_response(prompt, example_output, special_instruction, 3, fail_safe,
                                          __chat_func_validate, __chat_func_clean_up, True)
  save_to_time_cost_txt(run_gpt_prompt_agent_chat_summarize_relationship.__name__, old, time.time())
  if output != False:
    return output, [output, prompt, gpt_param, prompt_input, fail_safe]
  # ChatGPT Plugin ===========================================================
...
def run_gpt_prompt_agent_chat(maze, persona, target_persona,
  ...
  save_to_time_cost_txt(run_gpt_prompt_agent_chat.__name__, old, time.time())
  if output != False:
    return output, [output, prompt, gpt_param, prompt_input, fail_safe]
  # ChatGPT Plugin ===========================================================
...
def run_gpt_prompt_summarize_ideas(persona, statements, question, test_input=None, verbose=False):
  ...
  output = ChatGPT_safe_generate_response(prompt, example_output, special_instruction, 3, fail_safe,
                                          __chat_func_validate, __chat_func_clean_up, True)
  save_to_time_cost_txt(run_gpt_prompt_summarize_ideas.__name__, old, time.time())
  if output != False:
    return output, [output, prompt, gpt_param, prompt_input, fail_safe]
  # ChatGPT Plugin ===========================================================
...
def run_gpt_prompt_generate_next_convo_line(persona, interlocutor_desc, prev_convo, retrieved_summary, test_input=None, verbose=False):
  ...
  save_to_time_cost_txt(run_gpt_prompt_generate_next_convo_line.__name__, old, time.time())
  return output, [output, prompt, gpt_param, prompt_input, fail_safe]
...
def run_gpt_prompt_generate_whisper_inner_thought(persona, whisper, test_input=None, verbose=False):
  ...
  save_to_time_cost_txt(run_gpt_prompt_generate_whisper_inner_thought.__name__, old, time.time())
  return output, [output, prompt, gpt_param, prompt_input, fail_safe]
...
def run_gpt_prompt_planning_thought_on_convo(persona, all_utt, test_input=None, verbose=False):
  ...
  save_to_time_cost_txt(run_gpt_prompt_planning_thought_on_convo.__name__, old, time.time())
  return output, [output, prompt, gpt_param, prompt_input, fail_safe]
...
def run_gpt_prompt_memo_on_convo(persona, all_utt, test_input=None, verbose=False):
  ...
  save_to_time_cost_txt(run_gpt_prompt_memo_on_convo.__name__, old, time.time())
  return output, [output, prompt, gpt_param, prompt_input, fail_safe]
