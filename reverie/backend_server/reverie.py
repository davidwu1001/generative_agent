...
class ReverieServer:
  ...
  def start_server(self, int_counter):
    ...
    while (True):
      ...
      if int_counter == 0:
        break
      print(f'{self.curr_time.strftime("%B %d, %Y, %H:%M:%S")} steps: {self.step}')
  ...
  def open_server_server(self):
    ...
    while True:
      try:
        int_count = 360  # 每360时间步（1h）保存一次

        print(f"保存存档完成，继续run {int_count}")
        cost = time.time() - old
        print(f"共花费时间：{int(cost / 3600)}h, {int((cost % 3600) / 60)}m,{int(cost % 60)}s")
        self.save()  # 保存一次存档
        rs.start_server(int_count)
        print(f'{self.curr_time.strftime("%B %d, %Y, %H:%M:%S")}\n steps: {self.step}') # 打印当前时间
      ...
      # sim_command = sys.stdin.buffer.readline().decode('utf-8')
      print(sim_command)
...
def init(target):
  # 创建存档日志文件夹
  if not os.path.exists(os.path.join(log_path, target)):
    os.makedirs(os.path.join(log_path, target))
  # 清空time_cost.txt
  with open(os.path.join(log_path, 'time_cost.txt'), 'w') as f:
    f.truncate(0)

def archive_statistics(log_path, archive_name):
  import time
  ...
  arch_path = os.path.join(log_path, archive_name)

  def handle_time_cost_txt(log_path):
    sum_time, count_ = {}, {}

    with open(os.path.join(log_path, 'time_cost.txt'), 'r') as f:
      for line in f:
        func_name, time_cost = line.strip().split(" ")
        sum_time[func_name] = float(time_cost) if func_name not in sum_time else sum_time[func_name] + float(
          time_cost)
        count_[func_name] = 1 if func_name not in count_ else count_[func_name] + 1

    func_list = list(sum_time.keys())
    sum_time_list = list(sum_time.values())
    count_list = list(count_.values())
    avenage_time_list = [sum_time / count_list[idx] for idx, sum_time in enumerate(sum_time_list)]

    return func_list, sum_time_list, count_list, avenage_time_list

  def draw_bar(figsize, categories, values, title, xlabel, ylabel, arch_path, file_name, _int=False):
    ...
    plt.savefig(os.path.join(arch_path, file_name))
    plt.close()

  func_list, sum_time_list, count_list, avenage_time_list = handle_time_cost_txt(log_path)
  print(func_list, sum_time_list, count_list, avenage_time_list)
  result = {
    "func_list":func_list,
    "sum_time_list":sum_time_list,
    "count_list":count_list,
    "avenage_time_list":avenage_time_list
  }
  with open(os.path.join(arch_path, 'result.json'), 'a') as f:
    json.dump(result, f)
  draw_bar((12, 12), func_list, avenage_time_list, "函数执行统计", "函数", ylabel='平均执行时间', arch_path=arch_path,
           file_name='平均执行时间')
  draw_bar((12, 12), func_list, sum_time_list, "函数执行统计", "函数", ylabel='累计执行时间', arch_path=arch_path,
           file_name='累计执行时间')
  draw_bar((12, 12), func_list, count_list, "函数执行统计", "函数", ylabel='执行次数', arch_path=arch_path,
           file_name='执行次数', _int=True)


if __name__ == '__main__':
  log_path = "/data/wcc/generative_agent/log"

  """
  http://10.161.39.11:8000/replay/July1_the_ville_isabella_maria_klaus-step-3-20/100/
  
  base_the_ville_isabella_maria_klaus
  base_the_ville_n25
  25_February_13_2023_05_00_20
  25_February_13_2023_06_21_40
  25_February_13_2023_06_55_00
  25_February_13_2023_05_02_40
  25_February_13_2023_05_51_20
  25_February_13_2023_07_46_00
  25_February_13_2023_00_00_00做完初始计划
  25_February_13_2023_06_19_50
  25_February_13_2023_14_17_00
  25_February_13_2023_16_29_40
  25_February_13_2023_16_02_50
  25_February_13_2023_17_02_50
  3_February_13_2023_00_00_00
  3_February_13_2023_14_00_00
  3_February_13_2023_16_02_50
  3_February_13_2023_17_02_50
  
  3_February_17_2023_10_26_10
  
  google-chrome --headless --disable-gpu --disable-software-rasterizer --disable-dev-shm-usage --remote-debugging-port=9222 http://10.161.39.11:8000/simulator_home
  google-chrome --headless --disable-gpu --disable-software-rasterizer --disable-dev-shm-usage --remote-debugging-port=9222 --no-sandbox --disable-setuid-sandbox --disable-web-security --allow-running-insecure-content http://10.161.39.11:8000/simulator_home
  """
  # os.environ["HTTP_PROXY"] = "http://127.0.0.1:7890"
  # os.environ["HTTPS_PROXY"] = "http://.127.00.1:7890"


  origin = "3_February_17_2023_10_26_10"
  target = "qwen_test_25_10"
  init(origin)  # 初始化
  # origin = input("Enter the name of the forked simulation: ").strip()
  # target = input("Enter the name of the new simulation: ").strip()
  rs = ReverieServer(origin, target)
  rs.open_server()
  archive_statistics(log_path,origin)  # 统计存档
  exit()
